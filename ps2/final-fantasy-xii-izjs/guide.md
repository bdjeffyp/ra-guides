...
**Trip to the City the Other Day** (Stealing): Before killing Pallicant, steal from it! It has one item, the Gastrophetes, in its rare steal slot. Put on Thief's Cuffs on one or two characters and steal away while the non-stealers heal.
...
**Scalp the Golden Skull** (Stealing): The numerous Dead Bones that spawn in Nabreus Deadlands - Overlooking Eternity are the target for this achievement. The Golden Skullcap helmet is a rare steal from them. Put on Thief's Cuffs, perhaps two characters with them, and start stealing while another character is killing them. Since these enemies have other items to steal, this makes for a slight RNG nightmare, though the quantity of enemies certainly helps. If you clear out the zone of Dead Bones, head out two zones away and return to try again. If it takes a while, expect to gain a lot of experience.
...
**Vagrant Story** (Stealing): One of the toughest Entite type enemies, Vagrant Soul will wipe even high level parties. I ended up waiting to steal the Hunter's Blade from it until near the end when prepping for Trial Mode because it's Darkra and Feara are tough to handle. I set up a gambit on the non-stealer to do the healing and use Hi-Ethers if their MP < 10%. With a team that can handle 4000+ HP of damage, and a healer maintaining them, this rare steal should come fairly quickly. You will need to kill Vagrant Soul for the next one.

**Subterranean Bow Theft** (Stealing): Luxollid is the other toughest Entite type enemy, though slightly easier than Vagrant Soul. However, the setup for stealing the Sagittarius bow is the same.
...
**Hunter Extraordinaire** (Stealing): To get the Bone of Byblos from Yiazmat, you will need to set RNG position to 16 in the Advance for Rare (thanks to [jpc241](https://retroachievements.org/user/jpc421) for this detail) while IN Ridorana Cataract - Colosseum, but before approaching Yiazmat to start the fight. This is because after the cutscene plays for the start of the fight, RNG is advanced by that much as it basically reloads the zone (and all the chests and traps).
...
**Magick Seed** (Stealing): An excellent spot to set up RNG manipulation and steal a Baltoro's Seed from a magick pot is in Pharos Subterra - Penumbra North. There is a lone magick pot in the rooms after the Phoenix boss chamber. Go into the zone that the magick pot is alone, set up the RNG position so that the next steal is a rare steal, then steal! Easy! Run out to the boss chamber and then if you want to steal another Baltoro's Seed (or more), head one more zone away, go back and repeat! Having a stock of Baltoro's Seeds is a good idea for Trial Mode...
